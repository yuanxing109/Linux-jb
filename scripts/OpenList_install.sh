...
#
# Description:
#   A management script for OpenList (https://github.com/Yxiguan/OpenList_123)
...
# 配置部分
# GitHub 相关配置
GITHUB_REPO="Yxiguan/OpenList_123"
VERSION_TAG="beta"
VERSION_FILE="/opt/openlist/.version"
GH_DOWNLOAD_URL="${GH_PROXY}https://github.com/Yxiguan/OpenList_123/releases/latest/download"
...
INSTALL() {
  ...
  if [ -n "$proxy_input" ]; then
    ...
    # 如果不需要代理，直接使用默认链接
    GH_DOWNLOAD_URL="https://github.com/Yxiguan/OpenList_123/releases/latest/download"
  ...
  REAL_VERSION=$(curl -s "https://api.github.com/repos/Yxiguan/OpenList_123/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/' 2>/dev/null || echo "$VERSION_TAG")
...
SHOW_ABOUT() {
    ...
    echo -e "${GREEN_COLOR}│    主项目: https://github.com/Yxiguan/OpenList_123 │${RES}"
    echo -e "${GREEN_COLOR}│                                                    │${RES}"
